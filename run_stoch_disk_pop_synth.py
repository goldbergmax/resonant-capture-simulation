...
def change_param(i):
    ...
    sim.run_sim(disk_end, t_end, t_start_removal=t_start_removal, write_dir=path / f'{p}_{q}_results_{adiabatic_str}_{K}_{i}.pkl')
...
path = Path(f'results/stoch_disk')
os.makedirs(path, exist_ok=True)

pool = Pool(48)
df = pd.DataFrame(pool.map(change_param, range(N)))
df.to_csv(path / f'{p}_{q}_results_{adiabatic_str}_{K}.csv')